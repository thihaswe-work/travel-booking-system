...
import { createDestinationSchema, updateDestinationSchema, listDestinationsQuerySchema } from './destination.validation';

const router = Router();

router.get('/', validate({ query: listDestinationsQuerySchema }), asyncHandler(destinationController.list));
